...
gogooku3から移植し、gogooku5のJQuantsAsyncFetcherに適応させた版。
date軸とcode軸のAPIコール数・転送量を実測して最適な軸を決定する。

主な変更点:
- api_client を JQuantsAsyncFetcher に対応
- 認証ロジックを JQuantsAsyncFetcher.authenticate() に統一
- gogooku5 の既存構造に適合
"""
...
class AxisDeciderOptimized:
    """daily_quotes取得軸の自動選択器（gogooku5版）"""

    AxisChoice = Literal["by_date", "by_code"]

    def __init__(self, api_client=None, cache_dir: Path | None = None):
        """
        Args:
            api_client: JQuantsAsyncFetcher インスタンス（認証済み）
            cache_dir: 計測結果のキャッシュディレクトリ
        """
        self.api_client = api_client
        self.base_url = "https://api.jquants.com/v1"

        # キャッシュ設定
        self.cache_dir = cache_dir or Path("output/cache/axis_decision")
    ...
    async def get_optimal_axis(
        ...
        try:
            # api_clientが認証済みか確認
            if not self.api_client or not self.api_client.id_token:
                if self.api_client:
            ...
            logger.error(f"Failed to measure API efficiency: {e}", exc_info=True)
            # 計測失敗時はデフォルト戦略
            default_axis = self._get_default_axis(market_filter)
            return default_axis, {
                "axis": default_axis,
                "error": str(e),
                "timestamp": datetime.now().isoformat(),
    ...
    async def _measure_code_axis(
        ...
        for code in sample_codes[:50]:  # 最大50銘柄で計測
            params = {"code": code, "from": from_api, "to": to_api}
            pagination_key = None
            pages_for_code = 0
    ...
    def _decide_optimal_axis(
        self, date_metrics: dict, code_metrics: dict, market_filter: bool
        ...
        if date_pages <= self.threshold * effective_code_pages:
            return (
                "by_date",
                f"Date axis is more efficient ({date_pages} <= {self.threshold} * {effective_code_pages:.0f})",
            )
        else:
            return (
                "by_code",
                f"Code axis is more efficient ({effective_code_pages:.0f} < {date_pages} / {self.threshold})",
            )
...
async def test_axis_decider():
    ...
    from .jquants_async_fetcher import JQuantsAsyncFetcher
    ...
    # JQuantsAsyncFetcher を作成
    api_client = JQuantsAsyncFetcher(email=email, password=password)
    decider = AxisDeciderOptimized(api_client)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # 最適軸の決定
        axis, metrics = await decider.get_optimal_axis(session, market_filter=True)
