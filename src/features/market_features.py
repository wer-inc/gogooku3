...
class MarketFeaturesGenerator:
    ...
    def build_topix_features(self, topix_df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            z_score("mkt_ret_1d", self.z_score_window),
            z_score("mkt_vol_20d", self.z_score_window),
            z_score("mkt_bb_bw", self.z_score_window),
            z_score("mkt_dd_from_peak", self.z_score_window)
        ])
