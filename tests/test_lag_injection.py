...
class Approx:
    def __init__(self, expected, abs_tol=1e-7):
...
def test_lag_returns_no_future_data():
    """
    ...
    assert df.filter(pl.col("Date") == "2025-01-01")["lag_returns_1d"][0] is None  # First row null
    assert df.filter(pl.col("Date") == "2025-01-02")["lag_returns_1d"][0] == pytest.approx(0.01)  # T-1 value
    assert df.filter(pl.col("Date") == "2025-01-03")["lag_returns_1d"][0] == pytest.approx(-0.02)  # T-1 value
    assert df.filter(pl.col("Date") == "2025-01-04")["lag_returns_1d"][0] == pytest.approx(0.03)  # T-1 value


def test_rank_ret_uses_lagged_returns():
    """
...
def test_sector_features_use_lagged_returns():
    """
...
def test_lag_5d_correctness():
    ...
    # Verify T-5 lag
    assert df.filter(pl.col("Date") == "2025-01-06")["lag_returns_5d"][0] == pytest.approx(0.01)  # T-5
    assert df.filter(pl.col("Date") == "2025-01-07")["lag_returns_5d"][0] == pytest.approx(0.02)  # T-5


def test_lag_10d_correctness():
    """
...
def test_no_leakage_in_correlation():
    """
    ...
    # Filter out nulls
    df_clean = df.filter(pl.col("lag_returns_1d").is_not_null() & pl.col("rank_ret_prev_1d").is_not_null())
    ...
    assert abs(corr) < 0.5, f"Leakage detected: correlation = {corr:.4f} (should be < 0.5)"
