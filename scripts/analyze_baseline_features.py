...
def analyze_feature_importance(data_path='output/ml_dataset_future_returns.parquet'):
    ...
    for feature in all_features:
        ...
        for target_col, importance_df in feature_importance_dict.items():
            imp = importance_df[importance_df['feature'] == feature]['importance'].values
            if len(imp) > 0:
    ...
    aggregated_df = pd.DataFrame(aggregated_importance).sort_values('avg_importance', ascending=False)

    print("\nTop 20 Most Important Features (Averaged):")
    for idx, row in aggregated_df.head(20).iterrows():
        print(f"  {row['feature']}: avg={row['avg_importance']:.1f}, max={row['max_importance']:.1f}")
    ...
    for category, keywords in categories.items():
        ...
        if category_features:
            cat_importance = aggregated_df[aggregated_df['feature'].isin(category_features)]['avg_importance'].sum()
...
if __name__ == "__main__":
    import sys
    data_path = sys.argv[1] if len(sys.argv) > 1 else 'output/ml_dataset_future_returns.parquet'
