...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('hpo_simple.log')
    ]
)
logger = logging.getLogger(__name__)


def run_simple_hpo(n_trials: int = 5,
                   timeout: int = 300,
    ...
    """
    try:
        logger.info(f"🚀 Starting Simple HPO Optimization")
        ...
        best_params_formatted = optimizer.objective_func.format_best_params(study.best_params)
...
def get_study_status(study_name: str = "atft_simple_hpo",
                    storage_url: Optional[str] = None):
    """
...
def run_mock_hpo(n_trials: int = 3):
    ...
    try:
        ...
        import optuna
        from src.gogooku3.hpo import MultiHorizonObjective, MetricsExtractor
        import random
        import time
        ...
        def mock_objective(trial):
            ...
            logger.info(f"Trial {trial.number}: lr={learning_rate:.2e}, batch={batch_size}, d_model={d_model}")
            ...
            lr_penalty = abs(learning_rate - 3e-4) * 100  # Penalty for being far from 3e-4
            ...
            mock_metrics = {
                'rank_ic': {
                    ...
                    '5d': base_rank_ic * 0.8 * batch_bonus * model_bonus - lr_penalty * 0.05,
                ...
                'sharpe': {
                    ...
                    '5d': base_sharpe * 0.8 * batch_bonus * model_bonus - lr_penalty * 0.1,
            ...
            for horizon in mock_metrics['rank_ic']:
                mock_metrics['rank_ic'][horizon] = max(-0.1, min(0.3, mock_metrics['rank_ic'][horizon]))
            for horizon in mock_metrics['sharpe']:
                mock_metrics['sharpe'][horizon] = max(-1.0, min(2.0, mock_metrics['sharpe'][horizon]))
...
def main():
    ...
    parser.add_argument("--study-name", type=str, default="atft_simple_hpo",
                      help="Optuna study name (default: atft_simple_hpo)")

    # Storage configuration
    parser.add_argument("--storage", type=str,
                      help="Storage URL (e.g., sqlite:///output/hpo/optuna.db)")

    # Legacy option
    parser.add_argument("--mock", action="store_true",
                      help="Run mock HPO without actual training")
