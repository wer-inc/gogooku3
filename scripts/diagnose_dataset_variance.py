...
def analyze_per_day_variance(parquet_path: str):
    ...
    print(f"\n{'='*80}")
    print("Dataset Variance Diagnostic")
    print(f"{'='*80}")
    ...
    print(f"\n{'='*80}")
    print("Overall Statistics (Before Per-Day Grouping)")
    print(f"{'='*80}")
    ...
    print(f"\n{'='*80}")
    print("Per-Day Statistics (Cross-Sectional Analysis)")
    print(f"{'='*80}")
    ...
    for col in target_cols:
        ...
        print(f"\n{col}:")
        print(
            f"   Per-day mean: μ={mean_of_daily_means:.6f}, σ={std_of_daily_means:.6f}"
        )
        ...
        if len(zero_var_days) > 0:
            ...
            print("      Sample zero-variance days:")
        ...
        if mean_of_daily_stds < 0.01:
            ...
            print(
                "      This suggests cross-sectional normalization is killing variance!"
            )
    ...
    for stats in per_day_stats:
        ...
        print(
            f"   Ratio (should be ~1): {stats['mean_of_daily_stds'] / stats['overall_std']:.4f}"
        )
        print(
            f"   Zero-variance days:   {stats['zero_var_days']} / {stats['total_days']}"
        )

        # Diagnosis
        ratio = (
            stats["mean_of_daily_stds"] / stats["overall_std"]
            if stats["overall_std"] > 0
            else 0
        )
        if ratio < 0.1:
            print(
                f"   🚨 DIAGNOSIS: Cross-sectional variance is {ratio*100:.1f}% of overall variance"
            )
            print(
                "                 Likely cause: Dataset is already cross-sectionally normalized!"
            )
        elif ratio > 0.8:
            print(
                f"   ✅ DIAGNOSIS: Cross-sectional variance is preserved ({ratio*100:.1f}%)"
            )
        else:
            print(
                f"   ⚠️  DIAGNOSIS: Partial variance loss ({ratio*100:.1f}% preserved)"
            )

    # Check for normalization artifacts
    print(f"\n{'='*80}")
    print("Normalization Artifact Detection")
    print(f"{'='*80}")
    ...
    for col in target_cols:
        ...
        if abs(mean_val) < 0.01 and 0.95 < std_val < 1.05:
            ...
            print("      Dataset appears to be globally normalized (mean~0, std~1)")
        else:
            print(f"   Mean: {mean_val:.6f}, Std: {std_val:.6f}")
            print("   Not standard-normalized (different scale)")
