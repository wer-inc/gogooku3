...
def add_short_core_features(ss: pl.DataFrame) -> pl.DataFrame:
    ...
    return ss.with_columns(
        [
            ...
            (pl.col("ShortSellingVolume") / (pl.col("TotalVolume") + EPS)).alias(
                "ss_volume_ratio"
            ),
            pl.col("ShortSellingVolume").alias("ss_volume_abs"),
            # Activity proxy (volume * ratio)
            (pl.col("ShortSellingVolume") * pl.col("ShortSellingRatio") / 100.0).alias(
                "ss_activity"
            ),
        ]
    )
...
def add_short_statistical_features(ss: pl.DataFrame) -> pl.DataFrame:
    ...
    tmp = ss.with_columns(
        [
            ...
            (pl.col("ss_ratio") - pl.col("ss_ratio").shift(5).over("Code")).alias(
                "ss_momentum_5d"
            ),
            (pl.col("ss_ratio") - pl.col("ss_ratio").shift(10).over("Code")).alias(
                "ss_momentum_10d"
            ),
...
def add_short_extreme_detection(
    ss: pl.DataFrame, percentile_window: int = 252
) -> pl.DataFrame:
...
def add_short_zscore_features(ss: pl.DataFrame, z_window: int = 252) -> pl.DataFrame:
    ...
    ss = ss.with_columns(
        [
            # Rolling means
            pl.col("ss_ratio")
            .rolling_mean(z_window)
            .over("Code")
            .alias("_ss_ratio_mean"),
            pl.col("ss_volume_ratio")
            .rolling_mean(z_window)
            .over("Code")
            .alias("_ss_volume_ratio_mean"),
            pl.col("ss_activity")
            .rolling_mean(z_window)
            .over("Code")
            .alias("_ss_activity_mean"),
            # Rolling stds
            pl.col("ss_ratio")
            .rolling_std(z_window)
            .over("Code")
            .alias("_ss_ratio_std"),
            pl.col("ss_volume_ratio")
            .rolling_std(z_window)
            .over("Code")
            .alias("_ss_volume_ratio_std"),
            pl.col("ss_activity")
            .rolling_std(z_window)
            .over("Code")
            .alias("_ss_activity_std"),
        ]
    )
    ...
    ss = ss.with_columns(
        [
            (
                (pl.col("ss_ratio") - pl.col("_ss_ratio_mean"))
                / (pl.col("_ss_ratio_std") + EPS)
            ).alias("ss_ratio_z52"),
            (
                (pl.col("ss_volume_ratio") - pl.col("_ss_volume_ratio_mean"))
                / (pl.col("_ss_volume_ratio_std") + EPS)
            ).alias("ss_volume_ratio_z52"),
            (
                (pl.col("ss_activity") - pl.col("_ss_activity_mean"))
                / (pl.col("_ss_activity_std") + EPS)
            ).alias("ss_activity_z52"),
        ]
    )
...
def attach_adv20_and_scale(
    ss_feat: pl.DataFrame, adv20_df: pl.DataFrame | None
) -> pl.DataFrame:
    ...
    return ss_with_adv.with_columns(
        [
            ...
            (pl.col("ss_volume_abs") / (pl.col("ADV20_shares") + EPS)).alias(
                "ss_volume_to_adv20"
            ),
            # Balance scaled features (if available)
            (pl.col("ss_balance").fill_null(0) / (pl.col("ADV20_shares") + EPS)).alias(
                "ss_balance_to_adv20"
            ),
            # Turnover adjustment
            (pl.col("ss_activity") / (pl.col("ADV20_shares") + EPS)).alias(
                "ss_turnover_adj"
            ),
            # Liquidity regime (high/low ADV20)
            (pl.col("ADV20_shares") > pl.col("ADV20_shares").quantile(0.7).over("Date"))
            .cast(pl.Int8)
            .alias("ss_high_liquidity"),
        ]
    )
...
def combine_short_and_positions(
    short_df: pl.DataFrame, positions_df: pl.DataFrame | None
) -> pl.DataFrame:
...
def add_short_selling_block(
    ...
    *,
    ...
    if short_df is None or short_df.is_empty():
        ...
        null_exprs = [
            pl.lit(None).alias(c) for c in null_cols[:-1]
        ]  # All except is_ss_valid
        null_exprs.append(pl.lit(0).cast(pl.Int8).alias("is_ss_valid"))
