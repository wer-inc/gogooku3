...
def precision_at_k_pos(scores: torch.Tensor, labels: torch.Tensor, k: int) -> tuple[float, float]:
    """
...
def ndcg_at_k(scores: torch.Tensor, labels: torch.Tensor, k: int, *, beta: float = 1.0) -> float:
    """
...
def ndcg_random_baseline(labels: torch.Tensor, k: int, *, beta: float = 1.0) -> float:
    """
