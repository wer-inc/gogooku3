...
class FlowFeatures:
    ...
    def generate_flow_features(
        df: pl.DataFrame, weekly_flow_df: Optional[pl.DataFrame] = None
    ) -> pl.DataFrame:
        """
        投資フロー特徴量を生成（仕様準拠）
    ...
    def _merge_weekly_flow_data(
        df: pl.DataFrame, weekly_flow_df: pl.DataFrame
    ) -> pl.DataFrame:
        """
        ...
        missing_cols = [
            col for col in required_cols if col not in weekly_flow_df.columns
        ]
        if missing_cols:
            # Create dummy columns if missing
            for col in missing_cols:
                if col not in ["Code", "WeekEndDate"]:
                    weekly_flow_df = weekly_flow_df.with_columns(pl.lit(0.0).alias(col))
        ...
        df = df.join_asof(
            weekly_flow_df.select(
                [
                    ...
                    *[col for col in weekly_flow_df.columns if col.startswith("flow_")],
                ]
            ),
            on="meta_date",
    ...
    def _generate_simulated_flow_data(df: pl.DataFrame) -> pl.DataFrame:
        ...
        if ret_col in df.columns:
            ...
            df = df.with_columns(
                pl.lit(np.clip(np.random.randn() * 0.2, -1, 1)).alias(
                    "flow_proprietary_net_ratio"
                )
            )

            # Trust banks (steady accumulation)
            df = df.with_columns(
                pl.lit(np.clip(0.1 + np.random.randn() * 0.05, -1, 1)).alias(
                    "flow_trust_net_ratio"
                )
            )
        else:
            ...
            for investor in [
                ...
                df = df.with_columns(pl.lit(0.0).alias(f"flow_{investor}_net_ratio"))
        ...
        if existing_flow_cols:
            ...
            for investor in investor_types:
                ...
                if flow_col in df.columns:
                    # 5-day momentum
                    df = df.with_columns(
                        pl.col(flow_col).diff(5).alias(f"flow_momentum_{investor}_5d")
                    )

                    # 20-day momentum
                    df = df.with_columns(
                        pl.col(flow_col).diff(20).alias(f"flow_momentum_{investor}_20d")
                    )

            # Calculate divergence between investor types
            if (
                "flow_foreign_net_ratio" in df.columns
                and "flow_individual_net_ratio" in df.columns
            ):
                df = df.with_columns(
                    (
                        pl.col("flow_foreign_net_ratio")
                        - pl.col("flow_individual_net_ratio")
                    ).alias("flow_divergence_foreign_individual")
                )

            if (
                "flow_institution_net_ratio" in df.columns
                and "flow_individual_net_ratio" in df.columns
            ):
                df = df.with_columns(
                    (
                        pl.col("flow_institution_net_ratio")
                        - pl.col("flow_individual_net_ratio")
    ...
    def _add_flow_validity_flags(df: pl.DataFrame) -> pl.DataFrame:
        ...
        for col in main_flow_cols:
            if col in df.columns:
                df = df.with_columns(pl.col(col).is_not_null().alias(f"is_{col}_valid"))
        ...
        if any(col in df.columns for col in main_flow_cols):
            ...
            df = df.with_columns(validity_expr.alias("is_flow_data_valid"))

        return df

    @staticmethod
    def calculate_flow_impact_features(
        df: pl.DataFrame, price_col: str = "Close"
    ) -> pl.DataFrame:
        """
        フローの価格インパクト特徴量を計算
        ...
        for investor in investor_types:
            ...
            if flow_col in df.columns:
                ...
                df = df.with_columns(
                    (pl.col(flow_col) * pl.col(ret_col)).alias(
                        f"flow_price_impact_{investor}_0d"
                    )
                )

                # Lead impact (flow leads price by 1 day)
                df = df.with_columns(
                    (pl.col(flow_col) * pl.col(ret_col).shift(-1)).alias(
                        f"flow_price_impact_{investor}_lead1d"
                    )
                )
                ...
                df = df.with_columns(
                    (pl.col(flow_col) * pl.col(ret_col).shift(1)).alias(
                        f"flow_price_impact_{investor}_lag1d"
                    )
                )
    ...
    def add_flow_seasonality_features(df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "cal_is_month_end" in df.columns:
            ...
            for investor in ["foreign", "individual", "institution"]:
                ...
                if flow_col in df.columns:
                    df = df.with_columns(
                        (pl.col(flow_col) * pl.col("cal_is_month_end")).alias(
                            f"flow_month_end_{investor}"
                        )
                    )
        ...
        if "cal_is_quarter_end" in df.columns:
            ...
            for investor in ["foreign", "institution"]:
                ...
                if flow_col in df.columns:
                    df = df.with_columns(
                        (pl.col(flow_col) * pl.col("cal_is_quarter_end")).alias(
                            f"flow_quarter_end_{investor}"
                        )
                    )
