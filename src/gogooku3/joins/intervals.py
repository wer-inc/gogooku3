...
class BusinessCalendar:
    ...
    @staticmethod
    def next_business_day(date: datetime.date, calendar_df: pl.DataFrame) -> datetime.date:
        """次営業日を取得"""
...
class AsOfJoiner:
    ...
    def add_tplus1_effective_date(
        ...
        df = df.with_columns([
            pl.col(disclosed_time_col).str.strptime(pl.Time, "%H:%M:%S").alias("_time_parsed")
        ])
        ...
        if calendar_df is not None:
            ...
            df = df.with_columns([
                pl.col("effective_date_raw").alias("effective_date")
            ])
        else:
            df = df.with_columns([
                pl.col("effective_date_raw").alias("effective_date")
...
class FlowDataJoiner:
    ...
    def expand_flow_intervals(
        ...
        flow_sorted = flow_sorted.with_columns([
            ...
            (pl.col(published_date_col) + pl.duration(days=1)).alias("effective_start")
        ])
    ...
    def calculate_cross_features(
        ...
        df = df.with_columns([
            ...
            (
                ...
                (pl.col("px_returns_1d").std() / (pl.col("mkt_ret_1d").std() + ridge_lambda))
            ).over(
                pl.col("Code").list.slice(-beta_window, beta_window)
        ...
        # Alpha calculations
        df = df.with_columns([
            (pl.col("px_returns_1d") - pl.col("cross_beta_60d") * pl.col("mkt_ret_1d")).alias("cross_alpha_1d"),
            (pl.col("px_returns_5d") - pl.col("cross_beta_60d") * pl.col("mkt_ret_5d")).alias("cross_alpha_5d")
        ])
        
        # Relative strength
        df = df.with_columns([
            (pl.col("px_returns_5d") / (pl.col("mkt_ret_5d") + 1e-10)).alias("cross_rel_strength_5d")
        ])
        ...
        df = df.with_columns([
            (pl.col("cross_alpha_1d") * pl.col("mkt_bull_200")).alias("cross_alpha_vs_regime")
        ])
        
        # Idiosyncratic volatility ratio
        df = df.with_columns([
            (pl.col("px_volatility_20d") / (pl.col("mkt_vol_20d") + 1e-10)).alias("cross_idio_vol_ratio")
        ])
