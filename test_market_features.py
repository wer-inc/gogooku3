...
def create_sample_data():
    ...
    for code in ["1001", "1002", "1003"]:
        ...
        stock_returns = beta * topix_returns + alpha + np.random.normal(0, 0.015, n_days)
    ...
    stock_df = stock_df.with_columns([
        ...
        pl.col("Close").ewm_mean(span=20, adjust=False).over("Code").alias("ema_20"),
    ])
    
    stock_df = stock_df.with_columns([
        ((pl.col("ema_5") - pl.col("ema_20")) / pl.col("ema_20")).alias("ma_gap_5_20"),
        pl.col("Close").pct_change().over("Code").rolling_std(20).alias("volatility_20d"),
    ])
...
def test_market_features():
    ...
    expected_market_cols = [
        "mkt_ret_1d", "mkt_ret_5d", "mkt_vol_20d", "mkt_ema_20",
        "mkt_bb_bw", "mkt_dd_from_peak", "mkt_bull_200", "mkt_trend_up"
    ]
    ...
    for col in expected_cross_cols:
        if col in enhanced_df.columns:
            ...
            if not sample.is_empty():
                ...
                logger.info(f"   Sample values: {[f'{v:.4f}' if v is not None else 'null' for v in values]}")
        else:
            logger.warning(f"❌ {col} missing")
    ...
    logger.info(f"New features added: {len(enhanced_df.columns) - len(stock_df.columns)}")
