...
class TestFlowJoiner:
    ...
    def test_build_flow_intervals(self):
        ...
        assert first_start == expected_start, f"Expected {expected_start}, got {first_start}"
        ...
        if len(flow_intervals) > 1:
            ...
            assert first_end == expected_end, f"Expected {expected_end}, got {first_end}"
    ...
    def test_expand_flow_daily(self):
        ...
        assert len(impulse_check) == 0, "flow_impulse should be 1 only when days_since_flow=0"
    ...
    def test_no_future_leak(self):
        ...
        negative_days = result.filter(pl.col("days_since_flow") < -1)  # -1は未経験を示す
...
def test_integration_with_real_structure():
    ...
    trades_spec = pl.DataFrame({
        "PublishedDate": [datetime(2024, 1, 5) + timedelta(weeks=i) for i in range(52)],
        "Section": ["TSEPrime"] * 52,
