#!/bin/bash
# Enhanced Claude Code launcher with dynamic environment detection (gogooku5)
# Usage:
...
Your mission:
1. Run health check: tools/health-check.sh
2. Fix ALL critical issues immediately (priority: P0)
3. Address warnings and recommendations (priority: P1-P2)
4. Verify improvements by re-running health check
5. Report completion status

Start by running the health check with the Bash tool, create a todo list, and systematically fix all issues."
...
Start by running the health check and prioritizing improvements."
        else
            AUTO_PROMPT="✅ AUTONOMOUS MODE: Project healthy. Proactive optimization.

Your mission:
1. Review recent git commits and project structure
...
if [ "$GPU_UTILIZATION" != "unknown" ] && [ "$GPU_UTILIZATION" -lt 50 ]; then
    OPT_OPPORTUNITIES="${OPT_OPPORTUNITIES}
- GPU utilization is ${GPU_UTILIZATION}% - consider increasing batch size or enabling GPU-ETL"
fi
...
if [ "$CUDA_VERSION" != "unknown" ]; then
    ...
    if [ "$CUDA_MAJOR" -ge 12 ]; then
        OPT_OPPORTUNITIES="${OPT_OPPORTUNITIES}
- CUDA $CUDA_VERSION detected - RAPIDS/cuDF GPU-ETL available"
    fi
fi

SYSTEM_PROMPT="You are an autonomous AI developer working on gogooku5, a modular refactoring of the gogooku3 Japanese stock market prediction system.
...
💡 OPTIMIZATION OPPORTUNITIES:${OPT_OPPORTUNITIES}
...
- Bash for running commands (dataset builds, training, tests)
...
🎯 KEY PRINCIPLES:
- Respect separation of concerns (dataset generation ≠ model training)
- Use Polars for data processing, GPU-ETL when available
- Proper as-of joins (T+1 logic) for financial data
