...
class TechnicalIndicators:
    ...
    @staticmethod
    def add_returns(
        df: pl.DataFrame, periods: list[int] = [1, 5, 10, 20]
    ) -> pl.DataFrame:
        """リターン計算"""
        exprs = []
        for period in periods:
            exprs.extend(
                [
                    (pl.col("Close") / pl.col("Close").shift(period) - 1).alias(
                        f"px_returns_{period}d"
                    ),
                    ((pl.col("Close") / pl.col("Close").shift(period)).log()).alias(
                        f"px_log_returns_{period}d"
                    ),
                ]
            )
    ...
    def add_volatility(df: pl.DataFrame, windows: list[int] = [20, 60]) -> pl.DataFrame:
        ...
        for window in windows:
            ...
            df = df.with_columns(
                (
                    pl.col("px_returns_1d").rolling_std(window, min_periods=window)
                    * np.sqrt(252)
                ).alias(f"px_volatility_{window}d")
            )
            # 有効フラグ（十分なデータがある場合のみTrue）
            df = df.with_columns(
                pl.col(f"px_volatility_{window}d")
                .is_not_null()
                .alias(f"is_vol{window}_valid")
            )
        return df

    @staticmethod
    def add_realized_volatility(
        df: pl.DataFrame, window: int = 20, method: str = "parkinson"
    ) -> pl.DataFrame:
        ...
        if method == "parkinson":
            ...
            df = df.with_columns(
                (
                    park_var.rolling_mean(window, min_periods=window).sqrt()
                    * np.sqrt(252)
                ).alias(f"px_park_vol_{window}d")
            )
            df = df.with_columns(
                pl.col(f"px_park_vol_{window}d")
                .is_not_null()
                .alias(f"is_park_vol{window}_valid")
            )
        elif method == "garman_klass":
            # Garman-Klass: more complex formula using OHLC
            term1 = 0.5 * (pl.col("High") / pl.col("Low")).log().pow(2)
            term2 = (2 * np.log(2) - 1) * (pl.col("Close") / pl.col("Open")).log().pow(
                2
            )
            gk_var = term1 - term2
            df = df.with_columns(
                (
                    gk_var.rolling_mean(window, min_periods=window).sqrt()
                    * np.sqrt(252)
                ).alias(f"px_gk_vol_{window}d")
            )
            df = df.with_columns(
                pl.col(f"px_gk_vol_{window}d")
                .is_not_null()
                .alias(f"is_gk_vol{window}_valid")
            )
        elif method == "rogers_satchell":
            # Rogers-Satchell: handles drift
            rs_var = (pl.col("High") / pl.col("Close")).log() * (
                pl.col("High") / pl.col("Open")
            ).log() + (pl.col("Low") / pl.col("Close")).log() * (
                pl.col("Low") / pl.col("Open")
            ).log()
            df = df.with_columns(
                (
                    rs_var.rolling_mean(window, min_periods=window).sqrt()
                    * np.sqrt(252)
                ).alias(f"px_rs_vol_{window}d")
            )
            df = df.with_columns(
                pl.col(f"px_rs_vol_{window}d")
                .is_not_null()
                .alias(f"is_rs_vol{window}_valid")
            )
        else:
    ...
    def add_moving_averages(
        df: pl.DataFrame, windows: list[int] = [5, 20, 60, 200]
    ) -> pl.DataFrame:
        """移動平均（SMA/EMA）"""
        for window in windows:
            df = df.with_columns(
                [
                    ...
                    .alias(f"px_sma_{window}"),
                    pl.col("Close")
                    .ewm_mean(span=window, adjust=False)
                    .alias(f"px_ema_{window}"),
                ]
            )
            df = df.with_columns(
    ...
    @staticmethod
    def add_price_ratios(
        df: pl.DataFrame, windows: list[int] = [5, 20, 60]
    ) -> pl.DataFrame:
        """価格比率指標（仕様準拠 + 既存互換）"""
        exprs = []
        for window in windows:
            exprs.extend(
                [
                    (pl.col("Close") / (pl.col(f"px_sma_{window}") + 1e-12)).alias(
                        f"px_price_to_sma_{window}"
                    ),
                    (pl.col("Close") / (pl.col(f"px_ema_{window}") + 1e-12)).alias(
                        f"px_price_to_ema_{window}"
                    ),
                    (
                        (pl.col("px_sma_5") - pl.col(f"px_sma_{window}"))
                        / (pl.col(f"px_sma_{window}") + 1e-12)
                    ).alias(f"px_ma_gap_5_{window}"),
                    # 仕様: 乖離率 (Close - ema_k) / ema_k
                    (
                        (pl.col("Close") - pl.col(f"px_ema_{window}"))
                        / (pl.col(f"px_ema_{window}") + 1e-12)
                    ).alias(f"px_price_ema{window}_dev"),
                ]
            )
        ...
        exprs.extend(
            [
                (
                    (pl.col("px_ema_5") - pl.col("px_ema_20"))
                    / (pl.col("px_ema_20") + 1e-12)
                ).alias("px_ema_gap_5_20"),
                (
                    (pl.col("px_ema_20") - pl.col("px_ema_60"))
                    / (pl.col("px_ema_60") + 1e-12)
                ).alias("px_ema_gap_20_60"),
                (
                    (pl.col("px_ema_60") - pl.col("px_ema_200"))
                    / (pl.col("px_ema_200") + 1e-12)
                ).alias("px_ema_gap_60_200"),
                (pl.col("px_ema_5") > pl.col("px_ema_20"))
                .cast(pl.Int8)
                .alias("px_ema_cross_5_20"),
            ]
        )
    ...
    def add_trend_indicators(
        df: pl.DataFrame, slope_windows: list[int] = [10, 20, 60]
    ) -> pl.DataFrame:
        """EMAスロープ（pct_change(n=3)）を追加"""
        exprs = []
        for k in slope_windows:
            exprs.append(
                pl.col(f"px_ema_{k}").pct_change(3).alias(f"px_ema{k}_slope_3")
            )
        return df.with_columns(exprs)

    @staticmethod
    def add_volume_indicators(
        df: pl.DataFrame, windows: list[int] = [5, 20]
    ) -> pl.DataFrame:
        """出来高指標"""
        ...
        for window in windows:
            df = df.with_columns(
                [
                    pl.col("Volume")
                    .rolling_mean(window, min_periods=window)
                    .alias(f"px_volume_ma_{window}"),
                    (
                        pl.col("Volume")
                        / (
                            pl.col("Volume").rolling_mean(window, min_periods=window)
                            + 1e-12
                        )
                    ).alias(f"px_volume_ratio_{window}"),
                ]
            )
            df = df.with_columns(
                pl.col(f"px_volume_ma_{window}")
                .is_not_null()
                .alias(f"is_volume{window}_valid")
            )
        ...
        if "SharesOutstanding" in df.columns:
            exprs.append(
                (pl.col("Volume") / (pl.col("SharesOutstanding") + 1e-12)).alias(
                    "px_turnover_rate"
                )
            )
    ...
    def add_atr(df: pl.DataFrame, period: int = 14) -> pl.DataFrame:
        ...
        return df.with_columns(
            [atr.alias(f"px_atr_{period}"), natr.alias(f"px_natr_{period}")]
        )
    ...
    def add_adx(df: pl.DataFrame, period: int = 14) -> pl.DataFrame:
        ...
        dm_plus = (
            pl.when((high_diff > low_diff) & (high_diff > 0))
            .then(high_diff)
            .otherwise(0)
        )
        dm_minus = (
            pl.when((low_diff > high_diff) & (low_diff > 0)).then(low_diff).otherwise(0)
        )
    ...
    @staticmethod
    def add_stochastic(
        df: pl.DataFrame, period: int = 14, smooth: int = 3
    ) -> pl.DataFrame:
    ...
    @staticmethod
    def add_targets(
        df: pl.DataFrame, horizons: list[int] = [1, 5, 10, 20]
    ) -> pl.DataFrame:
        """予測ターゲット生成"""
        exprs = []
        ...
        for horizon in horizons:
            # Forward returns
            fwd_return = (pl.col("Close").shift(-horizon) / pl.col("Close") - 1).alias(
                f"y_{horizon}d"
            )
            exprs.append(fwd_return)

            # Binary targets (positive return)
            exprs.append((fwd_return > 0).cast(pl.Int8).alias(f"y_{horizon}d_bin"))
    ...
    def add_all_indicators(
        ...
        df = TechnicalIndicators.add_realized_volatility(
            df, window=20, method=realized_vol_method
        )
        df = TechnicalIndicators.add_moving_averages(df, ma_windows)
        df = TechnicalIndicators.add_price_ratios(
            df, [w for w in ma_windows if w <= 60]
        )
