...
class CodeNormalizer:
    ...
    @staticmethod
    def normalize_code(code: Union[str, int, None]) -> Optional[str]:
        """
        単一の銘柄コードを正規化
...
def apply_code_normalization_to_all_sources(
    daily_quotes: Optional[pl.DataFrame] = None,
    statements: Optional[pl.DataFrame] = None,
    trades_spec: Optional[pl.DataFrame] = None,
    listed_info: Optional[pl.DataFrame] = None,
    topix: Optional[pl.DataFrame] = None
) -> dict:
    """
    全データソースにCode正規化を適用
