...
class SafeJoinerV2:
    ...
    def prepare_base_quotes(self, quotes_df: pl.DataFrame) -> pl.DataFrame:
        ...
        quotes = self.code_normalizer.normalize_dataframe(
            quotes_df, code_columns=["Code"], target_column="Code"
        )
        ...
        if before_dedup != after_dedup:
            logger.warning(
                f"Removed {before_dedup - after_dedup} duplicate (Code, Date) pairs after normalization"
            )
        ...
        logger.info(
            f"Base quotes prepared: {len(quotes)} rows, {quotes['Code'].n_unique()} stocks"
        )
    ...
    def join_statements_with_dedup(
        ...
        stm = self.code_normalizer.normalize_dataframe(
            statements_df, code_columns=["LocalCode", "Code"], target_column="Code"
        )
        ...
        stm = (
            stm.sort(["Code", "DisclosedDate", "disclosed_ts"])
        ...
        if use_time_cutoff and "DisclosedTime" in stm.columns:
            ...
            stm = stm.with_columns(
                [
                    pl.when(pl.col("disclosed_ts").dt.time() < pl.col("cutoff_time"))
            ...
            stm = stm.with_columns(
                [
                    self._next_business_day_expr(pl.col("DisclosedDate")).alias(
                        "effective_date"
                    )
                ]
            )
        ...
        try:
            from src.features.statements_yoy import add_strict_yoy  # type: ignore
            ...
            try:
                ...
                logger.warning(f"Failed to apply strict YoY: {e}")
        ...
        # 統計情報
        coverage = result.filter(pl.col("stmt_days_since_statement") < 999).shape[
            0
        ] / len(result)
        self.join_stats["statements"] = {
            "coverage": coverage,
            "rows_with_data": result.filter(
                pl.col("stmt_days_since_statement") < 999
            ).shape[0],
    ...
    def join_trades_spec_with_gap_filling(
        ...
        flow = trades_df.with_columns(
            [pl.col("PublishedDate").cast(pl.Date), pl.col("Section").cast(pl.Utf8)]
        )

        # 同週複数公表の解決（最新のみ採用）
        flow = (
            flow.sort(["Section", "PublishedDate"])
        ...
        flow = flow.with_columns(
            [
                self._next_business_day_expr(pl.col("PublishedDate")).alias(
                    "effective_start"
                )
            ]
        )
        ...
        for section in base_df["Section"].unique():
            ...
            if section_flow.is_empty() or section == "AllMarket":
                ...
                section_df = section_df.sort("Date").join_asof(
                    section_flow.select(
                        ["effective_start", "effective_end"] + flow_cols
                    ),
                    left_on="Date",
                ...
                section_df = section_df.with_columns(
                    [
                        ...
                        .then(
                            (pl.col("Date") - pl.col("effective_start")).dt.total_days()
                        )
    ...
    def join_topix_with_validation(
        self, base_df: pl.DataFrame, topix_df: pl.DataFrame
    ) -> pl.DataFrame:
        """
        ...
        # TOPIXデータの準備
        topix = topix_df.with_columns([pl.col("Date").cast(pl.Date)])

        # mkt_* 特徴量の確認
        mkt_cols = [col for col in topix.columns if col.startswith("mkt_")]

        # 同日結合
        result = base_df.join(topix.select(["Date"] + mkt_cols), on="Date", how="left")
        ...
        if mkt_cols:
            ...
            self.join_stats["topix"] = {
                ...
                "cross_features": len(
                    [c for c in result.columns if c.startswith("cross_")]
                ),
    ...
    def _pad_calendar(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        calendar = pl.date_range(
            date_min, date_max, interval="1d", eager=True, closed="both"
        ).to_frame("Date")
    ...
    def _calculate_statement_features(self, stm: pl.DataFrame) -> pl.DataFrame:
        ...
        if required_stmt_cols.issubset(existing_stmt_cols):
            ...
            if (
                "stmt_yoy_sales" in existing_stmt_cols
                and "stmt_revenue_growth" not in existing_stmt_cols
            ):
                alias_exprs.append(
                    pl.col("stmt_yoy_sales").alias("stmt_revenue_growth")
                )
            if (
                "stmt_opm" in existing_stmt_cols
                and "stmt_profit_margin" not in existing_stmt_cols
            ):
        ...
        if "Code" not in s.columns and "LocalCode" in s.columns:
            s = s.with_columns(pl.col("LocalCode").cast(pl.Utf8).alias("Code"))
        ...
        if issued_col in s.columns:
            share_casts.append(
                ...
                .alias("stmt_issued_shares")
            )
        if treasury_col in s.columns:
            share_casts.append(
                pl.col(treasury_col)
                .cast(pl.Float64, strict=False)
                .alias("stmt_treasury_shares")
            )
        if avg_shares_col in s.columns:
            share_casts.append(
                pl.col(avg_shares_col)
                .cast(pl.Float64, strict=False)
                .alias("stmt_average_shares")
            )
        if share_casts:
        ...
        if "fiscal_year" in s.columns:
            try:
                non_null_count = s.select(
                    pl.col("fiscal_year").is_not_null().sum()
                ).item()
        ...
        if has_fiscal_year:
            try:
                ...
                drop_cols = [c for c in ("base_fy", "base_q") if c in s.columns]
                if drop_cols:
                    s = s.drop(drop_cols)
            except Exception as e:
        ...
        s = s.with_columns(
            [
                ...
                pl.col("ForecastOperatingProfit")
                .shift(1)
                .over("Code")
                .alias("prev_fore_op"),
                pl.col("ForecastProfit").shift(1).over("Code").alias("prev_fore_np"),
                pl.col("ForecastEarningsPerShare")
                .shift(1)
                .over("Code")
                .alias("prev_fore_eps"),
                pl.col("ForecastDividendPerShareAnnual")
                .shift(1)
                .over("Code")
                .alias("prev_fore_div"),
            ]
        ...
        s = s.with_columns(
            [
                pct_change(pl.col("NetSales"), pl.col("yoy_sales_base")).alias(
                    "stmt_yoy_sales"
                ),
                pct_change(pl.col("OperatingProfit"), pl.col("yoy_op_base")).alias(
                    "stmt_yoy_op"
                ),
                pct_change(pl.col("Profit"), pl.col("yoy_np_base")).alias(
                    "stmt_yoy_np"
                ),
                (pl.col("OperatingProfit") / (pl.col("NetSales") + 1e-12)).alias(
                    "stmt_opm"
                ),
                (pl.col("Profit") / (pl.col("NetSales") + 1e-12)).alias("stmt_npm"),
                (
                    pl.col("OperatingProfit")
                    / (pl.col("ForecastOperatingProfit") + 1e-12)
                ).alias("stmt_progress_op"),
                (pl.col("Profit") / (pl.col("ForecastProfit") + 1e-12)).alias(
                    "stmt_progress_np"
                ),
                pct_change(
                    pl.col("ForecastOperatingProfit"), pl.col("prev_fore_op")
                ).alias("stmt_rev_fore_op"),
                pct_change(pl.col("ForecastProfit"), pl.col("prev_fore_np")).alias(
                    "stmt_rev_fore_np"
                ),
                pct_change(
                    pl.col("ForecastEarningsPerShare"), pl.col("prev_fore_eps")
                ).alias("stmt_rev_fore_eps"),
                pct_change(
                    pl.col("ForecastDividendPerShareAnnual").cast(
    ...
    def _calculate_flow_features(self, flow: pl.DataFrame) -> pl.DataFrame:
        ...
        if "ForeignersBalance" in flow.columns:
            flow = flow.with_columns(
                [
                    (
                        pl.col("ForeignersBalance")
                        / (pl.col("ForeignersTotal") + 1e-12)
                    ).alias("flow_foreign_net_ratio"),
                    (
                        pl.col("IndividualsBalance")
                        / (pl.col("IndividualsTotal") + 1e-12)
