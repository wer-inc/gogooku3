...
config = OmegaConf.create(
    {
        "data": {
            "source": {"data_dir": "/home/ubuntu/gogooku3-standalone/output/atft_data"},
            "time_series": {
                "sequence_length": 20,
                "prediction_horizons": [1, 5, 10, 20],
            },
        },
        "train": {"batch": {"batch_size": 32}},
    }
)
        ...
        if isinstance(targets, dict):
            for k, v in targets.items():
                if torch.is_tensor(v):
                    ...
                    total = v.numel()
                    print(
                        f"    {k}: shape={v.shape}, nonzero={nonzero}/{total} ({100*nonzero/total:.1f}%)"
                    )
                    print(f"      mean={v.mean():.6f}, std={v.std():.6f}")
                    print(f"      sample values: {v.flatten()[:5].tolist()}")
        else:
            print(
                f"  Targets shape: {targets.shape if hasattr(targets, 'shape') else 'N/A'}"
            )
