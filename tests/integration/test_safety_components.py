...
def create_sample_data(n_samples=1000, n_features=10, n_days=50):
    ...
    logger.info(
        f"Created sample data: {len(df)} rows, {n_days} dates, {len(codes)} stocks"
    )
    return df
...
class TestCrossSectionalNormalizer:
    ...
    def test_basic_functionality(self):
        ...
        ), f"Normalization warnings: {validation['warnings']}"
    ...
    def test_data_leakage_prevention(self):
        ...
        normalizer = CrossSectionalNormalizer(date_column="date", code_column="code")
        ...
        future_feature_0 = test_normalized[
            test_normalized["date"] > df["date"].quantile(0.8)
        ]["feature_0"]
...
class TestFinancialMetrics:
    ...
    def test_ic_calculation(self):
        ...
        targets = (
            predictions * 0.8 + np.random.randn(n_samples) * 0.01
        )  # 80% correlation
        dates = np.repeat(
            pd.date_range(end=pd.Timestamp.now(), periods=25, freq="D"), 20
        )[:n_samples]
    ...
    def test_decile_analysis(self):
        ...
        dates = np.repeat(
            pd.date_range(end=pd.Timestamp.now(), periods=20, freq="D"), 20
        )[:n_samples]

        metrics_calc = FinancialMetrics(min_stocks_per_day=15, decile_count=5)

        decile_result = metrics_calc.compute_decile_analysis(
        ...
        print(
            f"✅ Decile analysis test passed: Long-Short={decile_result['long_short_spread']:.4f}"
        )
...
class TestIntegration:
    ...
    def test_full_pipeline(self):
        ...
        # 2. Normalize
        normalizer = CrossSectionalNormalizer(
            date_column="date", code_column="code", min_stocks_per_day=10
        )
        ...
        ic = metrics_calc.compute_information_coefficient(
            val_predictions, val_targets, val_dates
        )
...
def run_all_tests():
    ...
    try:
        ...
        success = (
            abs(ic) > 0.1
            and abs(rank_ic) > 0.1
            and integration_result["valid_days"] > 0
            and integration_result["pipeline_success"]
        )
