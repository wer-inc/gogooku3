...
class PurgedKFoldSplitter:
    ...
    def split(
        ...
        if array.size < self.n_splits:
            raise ValueError(
                f"Insufficient days ({array.size}) for {self.n_splits}-fold split"
            )
        ...
        if not (1 <= fold_index <= self.n_splits):
            raise ValueError(
                f"fold_index must be within [1, {self.n_splits}], got {fold_index}"
            )
        ...
        if train_idx.size and test_idx.size:
            ...
            assert (
                min_dist >= purge
            ), f"Leakage detected: min distance {min_dist} < purge {purge}"
