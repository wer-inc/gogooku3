...
def recompute_forward_targets(path: Path, output: Path) -> pl.LazyFrame:
    ...
    lf = add_future_return_columns(lf, horizons, price_col=price_col, code_col=code_col, date_col=date_col)
...
def _attach_fs(df: pl.DataFrame, *, settings: object) -> pl.DataFrame:
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--trim-tail", type=int, default=0, help="Drop last N calendar days per code to avoid missing forward targets"
    )
