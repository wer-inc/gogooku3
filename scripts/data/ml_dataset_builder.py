...
class MLDatasetBuilder:
        ...
        if {"High", "Low", "Close"}.issubset(df.columns) and "adx_14" not in df.columns:
            ...
            df = df.with_columns(
                (
                    ...
                    / ((pl.col("_plus_di") + pl.col("_minus_di")) + eps)
                    * 100.0
                ).alias("_dx")
            )
            df = df.with_columns(
        ...
        try:
            ...
            if not feat_ret_cols:
                validation_results["critical_errors"].append(
                    "No forward return labels found (feat_ret_* columns missing)"
                )
        ...
        try:
            ...
            try:
                ...
                if dup_cols:
                    ...
                    logger.info(
                        f"[builder] Dropped duplicate market cols prior to join: {dup_cols[:4]}{'...' if len(dup_cols)>4 else ''}"
                    )
            except Exception:
                pass
    ...
    def add_breakdown_features(
        ...
        *,
        ...
        feature_cols = [c for c in join_frame.columns if c not in {"Code", "Date"}]
    ...
    def add_fs_quality_features(
        ...
        *,
        ...
        if next_expr is not None:
            fs = fs.with_columns(next_expr(pl.col("Date")).alias("next_bday"))
        else:
    ...
    def add_sector_index_features(
        ...
        *,
        ...
        try:
            ...
            out = attach_sector_index_features(
                df,
                indices_df,
                listed_info_df,
                prefix=prefix,
                mask_halt_day=mask_halt_day,
            )
            return out
        except Exception as e:
            ...
            if {"returns_1d", "sec_ret_1d_eq"}.issubset(
                ...
                out = (
                    ...
                    .with_columns(
                        [
                            pl.col("returns_1d")
                            .rolling_mean(window_size=60, min_periods=1)
                            .over("Code")
                            .alias("x_mean"),
                            pl.col("sec_ret_1d_eq")
                            .rolling_mean(window_size=60, min_periods=1)
                            .over("Code")
                            .alias("y_mean"),
                            (pl.col("returns_1d") * pl.col("sec_ret_1d_eq"))
                            .rolling_mean(window_size=60, min_periods=1)
                            .over("Code")
                            .alias("xy_mean"),
                            pl.col("sec_ret_1d_eq")
    ...
    def add_sector_target_encoding(
        ...
        *,
        ...
        try:
                ...
                if "Close" in df.columns and "Code" in df.columns:
                    try:
                        if target_col == "target_5d":
                            ...
                            logger.warning(
                                f"{target_col} missing and cannot be derived; skipping sector target encoding"
                            )
                            return df
                    except Exception as exc:
            ...
            out = x.join(
                keys.select(["Date", id_col, "fold", te_col]),
                on=["Date", id_col, "fold"],
                how="left",
            )
            ...
            result = x.join(
                keys_nf.select(["Date", id_col, te_col]),
                on=["Date", id_col],
                how="left",
            )
    ...
    def finalize_for_spec(
        ...
        try:
            ...
            clip_exprs = [
                pl.col(col).clip(min_val, max_val).alias(col)
                for col, (min_val, max_val) in clip_map.items()
    ...
    def add_earnings_features(
        ...
        try:
            from src.gogooku3.features.earnings_features import (
                add_earnings_features as _add_earnings,
            )
            ...
            logger.info(
                "[builder] Added earnings features: days_to_earnings, days_since_earnings, is_earnings_week, etc."
            )
    ...
    def add_short_position_features(
        ...
        try:
            from src.gogooku3.features.short_features import (
                add_short_position_features as _add_shorts,
            )
    ...
    def add_enhanced_listed_features(
        ...
        try:
            from src.gogooku3.features.listed_features import (
                add_listed_info_features as _add_listed,
            )
            ...
            logger.info(
                "[builder] Added enhanced listed features: market_cap_log, liquidity_score, sector_momentum, etc."
            )
    ...
    def add_enhanced_margin_features(
        ...
        try:
            from src.gogooku3.features.margin_trading_features import (
                add_enhanced_margin_features as _add_margin,
            )
            ...
            logger.info(
                "[builder] Added enhanced margin features: margin_balance_ratio, margin_velocity, margin_stress_indicator, etc."
            )
    ...
    def add_option_sentiment_features(
        ...
        try:
            from src.gogooku3.features.option_sentiment_features import (
                add_option_sentiment_features as _add_options,
            )
            ...
            # Add features
            df_with_options = _add_options(df_renamed, fetcher, start_date, end_date)

            # Rename back
            df_with_options = df_with_options.rename({"date": "Date", "code": "Code"})

            logger.info(
                "[builder] Added option sentiment features: put_call_ratio, iv_skew, smart_money_indicator, etc."
            )
    ...
    def add_enhanced_flow_features(
        ...
        try:
            from src.gogooku3.features.enhanced_flow_features import (
                add_enhanced_flow_features as _add_flow,
            )
            ...
            # Add features
            df_with_flow = _add_flow(df_renamed, fetcher, start_date, end_date)

            # Rename back
            df_with_flow = df_with_flow.rename({"date": "Date", "code": "Code"})

            logger.info(
                "[builder] Added enhanced flow features: institutional_accumulation, foreign_sentiment, smart_flow_indicator, etc."
            )
            return df_with_flow
