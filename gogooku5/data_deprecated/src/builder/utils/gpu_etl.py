...
"""
GPU-ETL utilities for gogooku5 (optional, safe fallback to CPU).

This module is migrated from the legacy gogooku3 implementation
(`src/utils/gpu_etl.py`) so that gogooku5 can operate without importing
...
try:  # pragma: no cover - defensive
    ...
    def _noop_patch(*_args, **_kwargs) -> None:  # noqa: D401,ARG001
        """No-op patch function used when pynvjitlink is available."""

        return None

    pynvjitlink.patch.patch_numba_linker = _noop_patch  # type: ignore[assignment]
...
def _has_cuda() -> bool:
    ...
    try:
        ...
        try:
            ...
            return _cp.cuda.runtime.getDeviceCount() > 0  # type: ignore[no-any-return]
        except Exception:
            return False
...
def _attach_cupy_allocator(cp, rmm, logger: logging.Logger) -> bool:  # type: ignore[override]
    ...
    if allocator is not None:
        try:
            ...
            logger.debug("Failed to attach legacy CuPy allocator: %s", exc)
    ...
    try:
        ...
        logger.debug("Failed to attach new CuPy allocator: %s", exc)
        return False


def init_rmm_legacy(initial_pool_size: str | None = None) -> bool:
    """Initialize RMM memory pool (best-effort, legacy behavior).
    ...
    try:
        ...
        size_str = os.getenv("RMM_POOL_SIZE", "") if initial_pool_size is None else initial_pool_size
        ...
        if use_pool:
            ...
            if size_str:
                ...
                if size_bytes and size_bytes > 0:
                    ...
                    logger.info("RMM initialized with pool allocator, initial size=%s", size_str)
        else:
            # Dynamic allocation mode (cuda_async or pool_size=0)
            kwargs["pool_allocator"] = False
            logger.info("RMM initialized with dynamic allocation (allocator=%s, no pool)", allocator_mode)
        ...
        logger.warning("RMM init failed: %s", exc)
        ...
        try:
            ...
            logger.debug("Fallback RMM init also failed: %s", exc2)
        return False
...
def init_rmm(initial_pool_size: str | None = None) -> bool:
    ...
    try:
        ...
        size_str_env = os.getenv("RMM_POOL_SIZE", "")
        size_str = (initial_pool_size or size_str_env).strip()
        size_bytes = _parse_bytes(size_str)

        if allocator_mode == "cuda_async":
            if size_bytes and size_bytes > 0:
                try:
                    free_bytes, _total_bytes = cp.cuda.runtime.memGetInfo()  # type: ignore[attr-defined]
                    ...
                    if size_bytes >= max_allowed and max_allowed > 0:
                        logger.warning(
                            "Requested RMM pool (%s) exceeds available GPU memory (%.1f GB free). Clamping to %.1f GB.",
                    ...
                    logger.debug("Failed to query CUDA free memory: %s", exc)
            try:
                ...
                if attached:
                    logger.info(
                        "RMM cuda_async configured (pool=%s, size=%s)",
                        "on" if use_pool else "off",
                        size_str or "0",
                    )
                else:
                    logger.warning(
                ...
                logger.warning("cuda_async MR init failed, using legacy path: %s", exc)

        # Non-cuda_async or error path → legacy behavior
        return init_rmm_legacy(initial_pool_size)
    except Exception as exc:
        logger.warning("RMM modern init failed: %s", exc)
        return init_rmm_legacy(initial_pool_size)


def to_cudf(df: pl.DataFrame):
    ...
    if _has_cuda():
        try:
            ...
            if gdf is not None:
                ...
                # Normalize rank to [0, 1] similar to CPU implementation
