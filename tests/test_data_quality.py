...
from features.validity_flags import ValidityFlagManager
...
class DataQualityValidator:
    ...
    def check_code_date_uniqueness(df: pl.DataFrame) -> bool:
        ...
        if total_rows != unique_rows:
            duplicates = df.group_by(["Code", "Date"]).count().filter(
                pl.col("count") > 1
            )
            logger.error(f"❌ (Code, Date) uniqueness violated: {len(duplicates)} duplicates found")
    ...
    def check_flow_impulse_consistency(df: pl.DataFrame) -> bool:
        ...
        if "Section" in df.columns:
            ...
            df_with_prev = df_sorted.with_columns([
                pl.col("days_since_flow").shift(1).over(["Code", "Section"]).alias("prev_days")
            ])
            ...
            if len(bad_transitions) > 0:
                logger.warning(f"⚠️ Flow days monotonicity issues: {len(bad_transitions)} transitions")
                # これは警告レベル（新区間開始の可能性）
    ...
    def check_statement_days_non_negative(df: pl.DataFrame) -> bool:
        ...
        if len(negative_days) > 0:
            logger.error(f"❌ Negative stmt_days_since_statement: {len(negative_days)} rows")
            logger.error(f"  Sample: {negative_days.head(3).select(['Code', 'Date', 'stmt_days_since_statement'])}")
            return False
    ...
    def check_validity_flags_consistency(df: pl.DataFrame) -> bool:
        ...
        if "is_fully_valid" in df.columns:
            ...
            available_base_cols = [col for col in base_validity_cols if col in df.columns]
            ...
            if available_base_cols:
                ...
                if len(inconsistent) > 0:
                    issues.append(f"is_fully_valid inconsistent: {len(inconsistent)} rows")
    ...
    def check_coverage_thresholds(
        ...
        if "stmt_days_since_statement" in df.columns:
            stmt_coverage = df.filter(pl.col("stmt_days_since_statement") < 999).shape[0] / len(df)
            coverages["statements"] = stmt_coverage
            if stmt_coverage < min_stmt_coverage:
                logger.warning(f"⚠️ Statements coverage {stmt_coverage:.1%} < {min_stmt_coverage:.1%}")
        
        # Flow coverage
        if "days_since_flow" in df.columns:
            flow_coverage = df.filter(pl.col("days_since_flow") < 999).shape[0] / len(df)
            coverages["flows"] = flow_coverage
            if flow_coverage < min_flow_coverage:
                logger.warning(f"⚠️ Flow coverage {flow_coverage:.1%} < {min_flow_coverage:.1%}")
        ...
        if mkt_cols:
            ...
            if mkt_coverage < min_mkt_coverage:
                logger.warning(f"⚠️ Market coverage {mkt_coverage:.1%} < {min_mkt_coverage:.1%}")
...
def test_minimum_coverage_assertion():
    ...
    df = pl.DataFrame({
        "Code": ["1301"] * 100,
        "Date": [datetime(2024, 1, 1).date() + timedelta(days=i) for i in range(100)],
    ...
    # 厳しい閾値でのチェック
    result = validator.check_coverage_thresholds(df, min_stmt_coverage=0.3, min_flow_coverage=0.2)
