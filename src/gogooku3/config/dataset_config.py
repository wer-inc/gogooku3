...
class JQuantsAPIConfig(BaseSettings):
    ...
    min_available_date: str | None = Field(
        None, description="Subscription lower bound (YYYY-MM-DD)"
    )

    # Connection settings
    max_concurrent_fetch: int = Field(
        75, description="Maximum concurrent API requests (paid plan)"
    )
...
class GPUConfig(BaseSettings):
    ...
    cuda_visible_devices: str = Field(
        "0", alias="CUDA_VISIBLE_DEVICES", description="GPU device IDs"
    )
...
class FeatureFlagsConfig(BaseSettings):
    ...
    advanced_vol: bool = Field(
        True, description="Yang-Zhang volatility and VoV features"
    )
    advanced_features: bool = Field(
        True, description="Advanced T+0 features (RSI×Vol, momentum×volume, etc.)"
    )
...
class DatasetConfig(BaseSettings):
    ...
    start_date: str | None = Field(
        None, description="Start date (YYYY-MM-DD, default: end - 5 years)"
    )
    ...
    disable_halt_mask: bool = Field(
        False, description="Disable special halt-day masking (2020-10-01)"
    )
    futures_continuous: bool = Field(
        False, description="Enable continuous futures series"
    )
    ...
    def validate_and_compute_dates(self) -> DatasetConfig:
        ...
        if span_days < min_span:
            ...
            if self.jquants.min_available_date:
                min_available_dt = datetime.strptime(
                    self.jquants.min_available_date, "%Y-%m-%d"
                )
