...
def analyze_improvement_trajectory(df: pd.DataFrame):
    ...
    if len(df) > 0:
        ...
        logger.info(f"改善率: {(last_val_sharpe - first_val_sharpe) / abs(first_val_sharpe) * 100:.1f}%")
        ...
        logger.info(f"\n過学習チェック:")
        logger.info(f"最近10エポックの平均 - Train: {train_sharpe_trend:.4f}, Val: {val_sharpe_trend:.4f}")
        
        if train_sharpe_trend > val_sharpe_trend * 2:
            logger.warning("警告: 過学習の兆候があります")
            

def plot_training_curves(df: pd.DataFrame, save_path: str = "output/training_curves.png"):
    """トレーニング曲線をプロット"""
...
def generate_recommendations(df: pd.DataFrame):
    ...
    if len(df) > 0:
        ...
        if last_val_sharpe > 0.5:
            recommendations.append("✅ Sharpe比が良好です。アンサンブル学習を検討してください。")
        elif last_val_sharpe > 0.2:
            recommendations.append("📈 Sharpe比が改善しています。ハイパーパラメータの微調整を推奨。")
        elif last_val_sharpe > 0:
            recommendations.append("🔄 Sharpe比がプラスです。ポートフォリオ最適化を適用してください。")
        else:
            recommendations.append("⚠️ Sharpe比がまだマイナスです。データ品質の再確認が必要。")
        ...
        if abs(last_val_ic) < 0.01:
            recommendations.append("📊 ICが低すぎます。特徴量エンジニアリングの見直しを。")
    ...
    if last_val_sharpe > 0:
        ...
        recommendations.append(f"🎯 目標達成まで: あと{improvement_needed:.0f}%の改善が必要")
