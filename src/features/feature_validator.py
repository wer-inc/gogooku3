...
class FeatureValidator:
    ...
    def add_validity_flags(cls, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 各銘柄のrow_indexを追加
        df = df.with_columns([pl.int_range(0, pl.len()).over("Code").alias("row_idx")])
    ...
    def calculate_realized_volatility(
        cls, df: pl.DataFrame, window: int = 20, annualize: bool = True
    ) -> pl.DataFrame:
        """
    ...
    def calculate_beta_with_lag(
        ...
        df = df.with_columns(
            [
                ...
                # E[Y^2]
                cls.apply_min_periods_to_rolling(
                    df, "mkt_ret_lag", window, "std", min_periods=window
                )
                .over("Code")
                .pow(2)
                .alias("y_var"),
    ...
    def validate_no_leakage(cls, df: pl.DataFrame) -> dict[str, bool]:
        ...
        if (
            "stmt_imp_statement" in df.columns
    ...
    def remove_redundant_features(cls, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if cols_to_drop:
            logger.info(
                f"Removing {len(cols_to_drop)} redundant features: {cols_to_drop}"
            )
