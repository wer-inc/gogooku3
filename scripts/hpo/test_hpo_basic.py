...
def test_metrics_extractor():
    ...
    try:
        ...
        if metrics and isinstance(metrics, TrainingMetrics):
            logger.info(f"   Extracted metrics: RankIC={len(metrics.rank_ic)}, Sharpe={len(metrics.sharpe)}")
            logger.info(f"   Train loss: {metrics.train_loss:.3f}, Val loss: {metrics.val_loss:.3f}")
...
def test_hpo_optimizer_init():
    ...
    try:
        ...
        required_params = ['atft_d_model', 'learning_rate', 'batch_size', 'horizon_weights']
...
def test_integration():
    ...
    try:
        ...
        from src.gogooku3.hpo import MultiHorizonObjective, MetricsExtractor, ATFTHPOOptimizer

        # Initialize components
        objective = MultiHorizonObjective()
        extractor = MetricsExtractor()
        optimizer = ATFTHPOOptimizer(study_name="integration_test", n_trials=1, timeout=10)
