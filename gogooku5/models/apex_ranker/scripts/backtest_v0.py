    ...
    def run(
        ...
        *,
        ...
        for step_idx, date in enumerate(tqdm(dates, desc="Backtesting")):
            ...
            if not (len(pred_scores) == len(actual_returns) == len(stock_codes)):
                raise ValueError(
        ...
        results = {
            ...
            "avg_turnover": np.mean(self.history["turnover"])
            if len(self.history["turnover"])
            else 0.0,
            "n_days": len(dates),
...
def load_model_and_config(
    ...
    if not model_candidate.exists():
        raise FileNotFoundError(f"Model checkpoint not found: {model_candidate}")

    checkpoint = torch.load(model_candidate, map_location="cpu")
    config["_resolved_model_path"] = str(model_candidate)
...
def prepare_validation_data(
    ...
    print(
        f"[INFO] Selected {len(selection.features)} features, {len(selection.masks)} masks"
    )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Path to model checkpoint (.pt)"
    )
    parser.add_argument("--config", type=str, required=True, help="Path to config YAML")
    parser.add_argument(
        "--output", type=str, required=True, help="Output JSON path for results"
    )
    parser.add_argument(
        "--top-k", type=int, default=50, help="Number of stocks to hold (default: 50)"
    )
    parser.add_argument(
        "--horizon", type=int, default=20, help="Forecast horizon to use (default: 20)"
    )
    parser.add_argument(
        "--val-days",
        type=int,
        default=None,
        help="Override validation days (default: from config)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to save daily top-K selections (parquet)",
    )
    ...
    model, config = load_model_and_config(args.model, str(config_path))
    ...
    date_series = df.select(pl.col(date_col).unique().sort()).to_series()
    date_values = date_series.to_list()
    date_ints = np.asarray(date_series.to_numpy(), dtype="datetime64[D]").astype(
        "int64"
    )
    ...
    print(
        f"      Data loaded: {data_start_date} onwards (includes {lookback}d lookback)"
    )
    ...
    results_clean = {k: _to_builtin(v) for k, v in results.items() if k != "history"}
    resolved_model_path = config.get("_resolved_model_path", args.model)
    results_clean["model_path"] = resolved_model_path
    results_clean["config_path"] = str(config_path)
