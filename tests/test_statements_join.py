...
def create_sample_statements():
    ...
    return pl.DataFrame({
        ...
        "DisclosedTime": ["15:30:00", "14:00:00", "11:00:00", "16:00:00", "13:30:00"],
        ...
        "ForecastOperatingProfit": [40000000, 45000000, 12000000, 32000000, 38000000],
        ...
        "ChangesBasedOnRevisionsOfAccountingStandard": [None, None, None, None, "true"],
...
class TestStatementsJoin:
    ...
    def test_calculate_statement_features(self):
        ...
        expected_cols = [
            ...
            "stmt_rev_fore_op", "stmt_rev_fore_np",
            "stmt_roe", "stmt_roa",
            "stmt_nc_flag"
        ]
    ...
    def test_t1_effective_date(self):
        ...
        assert may10_code1301["stmt_imp_statement"][0] == 1, "Should be impulse day (before cutoff)"
    ...
    def test_no_future_leak(self):
        ...
        assert len(negative_days) == 0, f"Found {len(negative_days)} records with future leak"
...
def test_integration_statements():
    ...
    # リーク検査
    assert (result.filter(pl.col("stmt_days_since_statement") >= 0)["stmt_days_since_statement"] >= 0).all(), "Found future leak"
    ...
    print(f"  - Features added: {joiner.join_stats['statements'].get('features_added', 0)}")
