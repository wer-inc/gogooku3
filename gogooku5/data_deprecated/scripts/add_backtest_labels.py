...
def add_forward_returns(
    ...
    for horizon in horizons:
        ...
        df = df.with_columns(
            [
                ((pl.col(price_col).shift(-horizon) - pl.col(price_col)) / pl.col(price_col))
                .over(code_col)
                .alias(col_name)
            ]
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add forward returns to Phase 2 dataset for backtest evaluation")
    parser.add_argument(
        "--input",
        type=str,
        required=True,
    ...
    existing_labels = [col for col in df.columns if col.startswith("returns_") and col.endswith("d")]
    if existing_labels:
        LOGGER.warning(
