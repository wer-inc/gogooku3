...
class TestSectionMapper:
    ...
    def test_market_code_mapping(self):
        ...
        assert mapper.MARKET_TO_SECTION["0106"] == "JASDAQStandard"  # JASDAQ スタンダード
    ...
    def test_create_section_mapping_old_market(self):
        ...
        listed_info_df = pl.DataFrame({
            ...
            "MarketCode": ["0101", "0101", "0104", "0106", "0107"],  # 一部、一部、マザーズ、JASDAQ-S、JASDAQ-G
        ...
        for code, expected_section in expected_sections.items():
            ...
            assert actual == expected_section, f"Code {code}: expected {expected_section}, got {actual}"
    ...
    def test_create_section_mapping_new_market(self):
        ...
        listed_info_df = pl.DataFrame({
            ...
            "MarketCode": ["0111", "0111", "0113", "0112"],  # プライム、プライム、グロース、スタンダード
    ...
    def test_market_transition_period(self):
        ...
        listed_info_df = pl.DataFrame({
            ...
            "MarketCode": ["0101", "0111", "0104", "0113"],  # 一部→プライム、マザーズ→グロース
            "Date": [date(2022, 3, 31), date(2022, 4, 4), date(2022, 3, 31), date(2022, 4, 4)]
        })
        
        result = mapper.create_section_mapping(listed_info_df)
        
        # 1301: 東証一部 → プライム
        code_1301_old = result.filter((pl.col("Code") == "1301") & (pl.col("valid_from") < date(2022, 4, 4)))
        if not code_1301_old.is_empty():
            assert code_1301_old["Section"][0] == "TSE1st"
        
        code_1301_new = result.filter((pl.col("Code") == "1301") & (pl.col("valid_from") >= date(2022, 4, 4)))
        if not code_1301_new.is_empty():
            assert code_1301_new["Section"][0] == "TSEPrime"
        
        # 4661: マザーズ → グロース
        code_4661_old = result.filter((pl.col("Code") == "4661") & (pl.col("valid_from") < date(2022, 4, 4)))
        if not code_4661_old.is_empty():
            assert code_4661_old["Section"][0] == "TSEMothers"
        
        code_4661_new = result.filter((pl.col("Code") == "4661") & (pl.col("valid_from") >= date(2022, 4, 4)))
