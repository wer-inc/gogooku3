...
class EnhancedMarginTradingExtractor:
    ...
    def extract_features(self,
        ...
        if df_margin_daily is not None and not df_margin_daily.is_empty():
            df_features = self._process_daily_margin(df_base, df_margin_daily, df_listed)
        else:
            df_features = df_base
        
        # Add weekly margin features if available
        if df_margin_weekly is not None and not df_margin_weekly.is_empty():
            df_features = self._add_weekly_margin_features(df_features, df_margin_weekly)
    ...
    def _process_daily_margin(self,
                            df_base: pl.DataFrame,
        ...
        if long_col in df_merged.columns and short_col in df_merged.columns:
            df_merged = df_merged.with_columns([
                (pl.col(long_col) - pl.col(short_col)).alias('margin_net_outstanding'),
                (
        ...
        if 'margin_buy_velocity' in df_merged.columns and 'margin_sell_velocity' in df_merged.columns:
            df_merged = df_merged.with_columns(
                (pl.col('margin_buy_velocity') - pl.col('margin_sell_velocity')).alias('margin_divergence')
            )
        ...
        if long_col in df_merged.columns:
            ...
            df_merged = df_merged.with_columns(
                ((pl.col(long_col) - pl.col('margin_buy_ma')) /
                 (pl.col('margin_buy_ma') + 1e-10)).alias('margin_momentum')
            )
    ...
    def _add_weekly_margin_features(self,
                                   df_features: pl.DataFrame,
                                   df_weekly: pl.DataFrame) -> pl.DataFrame:
        ...
        if 'LongMarginTradeVolume' in df_merged.columns:
            ...
            # Normalize to 0-1
            df_merged = df_merged.with_columns(
                (pl.col('institutional_margin_rank') / pl.col('institutional_margin_rank').max())
    ...
    def _calculate_composite_indicators(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if stress_components:
            ...
            for comp in stress_components:
                ...
                df = df.with_columns(
                    (pl.col(f'{comp}_rank') / pl.col(f'{comp}_rank').max()).alias(f'{comp}_norm')
                )
    ...
    def _normalize_by_market_cap(self,
                                df: pl.DataFrame,
        ...
        df = df.join(
            df_listed.select(['code', 'market_cap']),
            on='code',
            how='left'
        )
...
def add_enhanced_margin_features(df_base: pl.DataFrame,
                                fetcher: Any,
                                start_date: str,
                                end_date: str,
                                use_weekly: bool = True) -> pl.DataFrame:
