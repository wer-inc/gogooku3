...
def create_sample_data():
    ...
    for code in ["1301", "5401", "7203"]:  # 3銘柄
        for date in dates:
            stock_data.append({"Code": code, "Date": date.date(), "Close": 1000.0})
...
def test_current_join_method():
    ...
    # 銘柄にSectionを付与（簡易版）
    stock_df = stock_df.with_columns(pl.lit("TSEPrime").alias("Section"))

    # 完全一致結合
    result = stock_df.join(
        flow_df.select(
            ["effective_date", "flow_foreign_net_ratio", "flow_smart_money_idx"]
        ),
        left_on="Date",
    ...
    logger.info(
        f"フロー特徴量が付与された行数: {result['flow_foreign_net_ratio'].is_not_null().sum()}"
    )
...
def test_improved_join_method():
    ...
    flow_df = flow_df.with_columns(
        [
            (pl.col("PublishedDate") + timedelta(days=3)).alias(
                "effective_start"
            ),  # 翌月曜
            (pl.col("PublishedDate") + timedelta(days=9)).alias(
                "effective_end"
            ),  # 次の金曜
        ]
    )
    ...
    for row in flow_df.iter_rows(named=True):
        mask = (result["Date"] >= row["effective_start"]) & (
...
def compare_results():
    ...
    logger.info("\n=== 比較結果 ===")
    logger.info(
        f"現在の方法: {current['flow_foreign_net_ratio'].is_not_null().sum()}/{len(current)} 行にデータあり"
    )
    logger.info(
        f"改善版: {improved['flow_foreign_net_ratio'].is_not_null().sum()}/{len(improved)} 行にデータあり"
    )

    # カバレッジの改善率
    current_coverage = current["flow_foreign_net_ratio"].is_not_null().sum() / len(
        current
    )
    ...
    for week in range(3):
        ...
        current_week = current.filter(
            (pl.col("Date") >= week_start.date()) & (pl.col("Date") <= week_end.date())
        )
        ...
        logger.info(
            f"  現在: {current_week['flow_foreign_net_ratio'].is_not_null().sum()}/{len(current_week)} 行"
        )
        logger.info(
            f"  改善: {improved_week['flow_foreign_net_ratio'].is_not_null().sum()}/{len(improved_week)} 行"
        )
