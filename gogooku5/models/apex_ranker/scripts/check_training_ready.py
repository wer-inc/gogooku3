...
def main() -> None:
    ...
    distinct_dates = (
        scan.select(pl.col(data_cfg["date_column"]).n_unique()).collect().item()
    )
    ...
    if selection.masks:
        coverage_exprs = [
            pl.col(mask).fill_null(0).gt(0.5).mean().alias(mask)
        ...
        for mask in selection.masks:
            ...
            if ratio == 0.0:
                print(
                    f"[WARN] Mask '{mask}' has zero positive coverage; it will be ignored."
                )
            else:
                pct = ratio * 100.0
