...
class DatasetBuilderSettings(BaseSettings):
    ...
    Values are resolved from environment variables (see `.env.example`)
    with sane defaults so local development can start quickly.
    """
    ...
    def latest_dataset_path(self) -> Path:
        ...
        return self.data_output_dir / "ml_dataset_latest.parquet"
