...
def _build_chunk_dataframe(
    ...
    *,
    ...
    if spec.date_kind == "string":
        date_expr = pl.col(spec.date_column).str.strptime(pl.Date, format=spec.date_format, strict=False)
    elif spec.date_kind == "datetime":
        date_expr = pl.col(spec.date_column).dt.date()
    else:
        date_expr = pl.col(spec.date_column).cast(pl.Date)

    lf = lf.with_columns(date_expr.alias(CHUNK_DATE_COL)).filter(pl.col(CHUNK_DATE_COL).is_not_null())
...
def _chunk_source(
    ...
    *,
    ...
    partitions = df.partition_by(CHUNK_ID_COL, maintain_order=True, include_key=False, as_dict=True)
    if not partitions:
        print(f"[WARN] {spec.name}: no chunkable partitions found")
        return

    print(f"[INFO] {spec.name}: {len(partitions)} quarter partitions " f"from {base_path.name}")
    written = 0
