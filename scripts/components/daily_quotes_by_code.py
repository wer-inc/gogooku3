...
class MarketMembership:
    ...
    def __init__(self):
        ...
        self.memberships: dict[str, list[tuple[str, str, str | None]]] = {}
    ...
    def add_membership(
        ...
        # 既存の期間と重複チェック
        for i, (mc, fd, _td) in enumerate(self.memberships[code]):
...
class DailyQuotesByCodeFetcher:
    ...
    async def fetch_multiple_codes(
        ...
        for i in range(0, total_codes, batch_size):
            ...
            for code, df in zip(batch_codes, results, strict=False):
                if not df.is_empty():
                    all_dfs.append(df)
